...
{
    public partial class frmBsPort : DockContent
    {
        ...
        private void frmBsPort_Load(object sender, EventArgs e)
        {  
            Load_Date();
            //系統狀態
            ...
            luecountry.DataBindings.Add("EditValue", bds1, "country");
        ...
        {
            dtDetail.Clear();           
            const string sql = @"SELECT * From bs_port with(nolock) Where state='0'";
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dgvDetails.Rows[i].Cells["id"].Value.ToString() == "" || dgvDetails.Rows[i].Cells["name"].Value.ToString() == "" || dgvDetails.Rows[i].Cells["country"].Value.ToString() == "")
                {
                    flag_datavalid = true;
            ...
            if (flag_datavalid)
            {
                MessageBox.Show("Port id cannot be empty !", myMsg.msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
        ...
        private void frmBsPort_FormClosed(object sender, FormClosedEventArgs e)
        {
           dtDetail=null ;
           SDA = null;
           conn.Close();
        ...
        {
            if (!string.IsNullOrEmpty(luecountry.Text))
            {
                txtcountry_name.Text = DBUtility._language == "2" ? luecountry.GetColumnValue("english_name").ToString() : luecountry.GetColumnValue("name").ToString();
            }
        }
    }
}
