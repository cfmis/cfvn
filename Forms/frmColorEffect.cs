...
{
    public partial class frmColorEffect : DockContent
    {
        ...
        private void frmColorEffect_Load(object sender, EventArgs e)
        {
            Find();
        }
        ...
        private bool Valid_Doc() //主建是否已存在
        {
            ...
            string strSql = String.Format("Select '1' FROM bs_color_effect WHERE id='{0}'", doc);
        ...
        private void Find_doc(string temp_id) //非新增或編號狀態下帶出的資料
        {
            ...
            {
                string strsql = String.Format("SELECT id,name,english_name,remark,create_by,create_date,update_by,update_date FROM dbo.bs_color_effect WHERE id='{0}'", temp_id);
                DataTable dt = clsApp.GetDataTable(strsql);
        ...
        private void Save()  //保存新增或修改的資料
        {
            ...
            if (mState == "NEW")
            {
                ...
                const string sql_new = "INSERT INTO bs_color_effect(id,name,english_name,remark,create_by,create_date,update_by,update_date) values(@id,@name,@english_name,@remark,@user_id,GETDATE())";
            ...
            if (mState == "EDIT")
            {
                const string sql_update = "UPDATE bs_color_effect SET name=@name,english_name=@english_name,remark=@remark,update_by=@user_id,update_date=GETDATE() WHERE id=@id";
                SqlConnection myCon = new SqlConnection(DBUtility.ConnectionString);
                myCon.Open();
                SqlTransaction myTrans = myCon.BeginTransaction();
                using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
        ...
        private void Delete() //刪除當前行
        {
            ...
            {
                const string sql_del = "DELETE FROM dbo.bs_color_effect WHERE id=@id";
                SqlConnection myCon = new SqlConnection(DBUtility.ConnectionString);
                myCon.Open();
                SqlTransaction myTrans = myCon.BeginTransaction();
                using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
        ...
        {            
            DataTable dt = clsApp.GetDataTable("SELECT * FROM dbo.bs_color_effect");
            dgvDetails.DataSource = dt;
        }
