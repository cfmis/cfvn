...
{
    public partial class frmBsTackFare : DockContent

    {
        ...
        private void frmBsTackFare_Load(object sender, EventArgs e)
        {
            //系統狀態
        ...
        private void frmBsTackFare_FormClosed(object sender, FormClosedEventArgs e)
        {
            dtCountry.Dispose();
        }
        ...
        {
            bool flag = false;
            string strSql = String.Format("Select '1' FROM bs_tack_fare WHERE id='{0}'", txtid.Text.Trim());
        ...
        private void Find_Doc(string temp_id) //非新增或編號狀態下帶出的資料
        {
            if (!String.IsNullOrEmpty(temp_id))
            {
                ...
                @"SELECT * FROM dbo.bs_tack_fare Where state='0' order by id ", temp_id);
        ...
        {
            dtDetail.Clear();
            const string sql = @"SELECT * From bs_tack_fare with(nolock)";           
        ...
        private void Save()  //保存新增或修改的資料
        {
            ...
            if (flag_datavalid)
            {
                MessageBox.Show("Surcharge ID cannot be empty !", myMsg.msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
