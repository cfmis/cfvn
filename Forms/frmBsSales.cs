...
{
    public partial class frmBsSales : DockContent
    {
        ...
        private void frmBsSales_Load(object sender, EventArgs e)
        {  
            Load_Date();
            //系統狀態
            ...
            luegroup.DataBindings.Add("EditValue", bds1, "sales_group");
        ...
        {
            const string strSql = @" SELECT id,english_name,name FROM dbo.bs_group ";
            DataTable dtType = clsApp.GetDataTable(strSql);
            luegroup.Properties.DataSource = dtType;
            luegroup.Properties.DisplayMember = "id";
        ...
        {
            dtDetail.Clear();           
            const string sql = @"SELECT * From bs_sales with(nolock) Where state='0'";
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (flag_datavalid)
            {
                MessageBox.Show("Sales id cannot be empty !", myMsg.msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
        ...
        private void frmBsSales_FormClosed(object sender, FormClosedEventArgs e)
        {
           dtDetail=null ;
           SDA = null;
           conn.Close();
