...
{
	public partial class frmDelivery : DockContent
	{
        ...
        DataTable dtCostprice = new DataTable();
        ...
        private void Print()
        {
            ...
            DataTable dtReprot = clsApp.GetDataTable(strSql);
        ...
        void Count_Head_Cost()
        {
            ...
            for (int i = 0; i < dgvDetails.RowCount; i++)
            {
                if (string.IsNullOrEmpty(dgvDetails.GetRowCellValue(i, "amt_total").ToString()))
                    temp_amt_total = 0;
                else
                    temp_amt_total = clsPub.Return_Float_Value(dgvDetails.GetRowCellValue(i, "amt_total").ToString());
                if (string.IsNullOrEmpty(dgvDetails.GetRowCellValue(i, "amt_other").ToString()))
                    temp_amt_other = 0;
                else
        ...
        private void txtcustomer_name_Leave(object sender, EventArgs e)
        {
            if (mState != "" && txtcustomer_name.Text != "")
            {
                string sql = string.Format(
                @"select customer_address,contact,customer_tel,customer_fax,province
                from dbo.customer_info where customer_name='{0}'", txtcustomer_name.Text);
                DataTable dt = new DataTable();
                dt = clsApp.GetDataTable(sql);
                if (dt.Rows.Count > 0)
                {
                    txtcustomer_address.Text = dt.Rows[0]["customer_address"].ToString();
                    txtcustomer_tel.Text = dt.Rows[0]["customer_tel"].ToString();
                    txtcustomer_fax.Text = dt.Rows[0]["customer_fax"].ToString();
                    txtcontact.Text = dt.Rows[0]["contact"].ToString();
                    lkeProvince.EditValue = dt.Rows[0]["province"].ToString();
                    if (lkeProvince.Text != "")
                    {
                        Count_Cost_Price(lkeProvince.Text, clsPub.Return_Float_Value(txtWeightCost.Text));
                    }
                }
                dt.Dispose();
            }
        ...
        private void Count_Cost_Price(string strProvince,float fltWeight)
        {
            if (fltWeight <= 0)
            {
                dtCostprice.Clear();
            ...
            SqlParameter[] paras = new SqlParameter[]{
                new SqlParameter("@strProvince",strProvince),
                new SqlParameter("@weight_budget",fltWeight)
            };
            dtCostprice = clsApp.ExecuteProcedureReturnTable("usp_get_cost_price_budget", paras);           
            dgvCostPrice.DataSource = dtCostprice;
        }

        private void dgvCostPrice_RowPostPaint(object sender, DataGridViewRowPostPaintEventArgs e)
        {
            if (dgvCostPrice.Rows[e.RowIndex].Cells["islarge"].Value.ToString() == "大件货")
            {
                dgvCostPrice.Rows[e.RowIndex].DefaultCellStyle.BackColor = Color.LightBlue;
            }
        }

        private void lkeProvince_Leave(object sender, EventArgs e)
        {
            if (mState != "" &&lkeProvince.Text !="")
            {
                Count_Cost_Price(lkeProvince.EditValue.ToString(), clsPub.Return_Float_Value(txtWeightCost.Text));
            }
        }

        private void txtWeightCost_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (clsPub.Return_Float_Value(txtWeightCost.Text) <= 0)
                {
                    return;
                }
                if (lkeProvince.Text == "")
