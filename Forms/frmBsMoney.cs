...
{
    public partial class frmBsMoney : DockContent

    {
        ...
        private void Bingding_Data()
        {
            ...
            txtcurrency_sign.DataBindings.Add("Text", bds1, "currency_sign");
            txtCountry_id.DataBindings.Add("EditValue", bds1, "country_id");
            luestate.DataBindings.Add("EditValue", bds1, "state");
            txtBp_formula.DataBindings.Add("Text", bds1, "bp_formula");
            txtSunaccount_rate.DataBindings.Add("Text", bds1, "sunaccount_rate");
        ...
        private void frmBsMoney_FormClosed(object sender, FormClosedEventArgs e)
        {
            dtCountry.Dispose();
        }
        ...
        {
            bool flag = false;
            string strSql = String.Format("Select '1' FROM bs_money WHERE id='{0}'", txtid.Text.Trim());
        ...
        {
            dtDetail.Clear();
            const string sql = @"SELECT * From bs_money with(nolock)";           
        ...
        private void Save()  //保存新增或修改的資料
        {
            ...
            if (flag_datavalid)
            {
                MessageBox.Show("Money id/name/country cannot be empty !", myMsg.msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
