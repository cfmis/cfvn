...
{
    public partial class frmBsPayment : DockContent
    {
        ...
        public clsAppPublic clPub = new clsAppPublic();
        ...
        private void frmBsPayment_Load(object sender, EventArgs e)
        {
            //系統狀態
        ...
        private bool Valid_Doc() //主建是否已存在
        {
            ...
            string strSql = String.Format("Select '1' FROM dbo.bs_payment WHERE id='{0}' and state<>'2'", doc);
        ...
        private void Find_doc(string temp_id) //非新增或編號狀態下帶出的資料
        {
            ...
            {
                string strsql = String.Format("SELECT * FROM dbo.bs_payment WHERE id='{0}' and state='0'", temp_id);
                DataTable dt = clsApp.GetDataTable(strsql);
        ...
        private void Set_Master_Data(DataTable dt)
        {
            txtID.Text = dt.Rows[0]["id"].ToString();
            txtenglish_name.Text = dt.Rows[0]["name"].ToString();
            txtname.Text = dt.Rows[0]["english_name"].ToString();
            if (dt.Rows[0]["count_thismonth"].ToString()=="True")
                chkcount_thismonth.Checked = true;
            else
                chkcount_thismonth.Checked = false;
            txtcountday.Text = dt.Rows[0]["countday"].ToString();
            txtdisc_day.Text = dt.Rows[0]["disc_day"].ToString();
            txtdisc_rate.Text = dt.Rows[0]["disc_rate"].ToString();
            txtRemark.Text = dt.Rows[0]["remark"].ToString();
            luestate.EditValue = dt.Rows[0]["state"].ToString();
        ...
        private void Save()  //保存新增或修改的資料
        {
            ...
            if (mState == "NEW" || mState == "EDIT")
            {
                ...
                @"INSERT INTO bs_payment(id,name,english_name,count_thismonth,countday,disc_day,disc_rate,interest_rate,state,remark,create_by,create_date) 
                  Values(@id,@name,@english_name,@count_thismonth,@countday,@disc_day,@disc_rate,@interest_rate,'0',@remark,@user_id,getdate())";
                const string sql_update =
                 @"UPDATE dbo.bs_payment SET name=@name,english_name=@english_name,count_thismonth=@count_thismonth,@countday=countday,disc_day=@disc_day,
                  disc_rate=@disc_rate,interest_rate=@interest_rate,remark=@remark,update_by=@user_id,update_date=getdate()
                WHERE id=@id";
                if (mState == "NEW")
                {
                    if (Valid_Doc())
                ...
                using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
                {
                    try
                    {
                        ...
                        myCommand.Parameters.AddWithValue("@count_thismonth", chkcount_thismonth.Checked ?true :false );
                        myCommand.Parameters.AddWithValue("@countday", txtcountday.Text);
                        myCommand.Parameters.AddWithValue("@disc_day", clPub.Return_Int_Value(txtdisc_day.Text));
                        myCommand.Parameters.AddWithValue("@disc_rate", clPub.Return_Float_Value(txtdisc_rate.Text));                    
                        myCommand.Parameters.AddWithValue("@interest_rate", clPub.Return_Float_Value(txtinterest_rate.Text));                       
        ...
        private void Delete() //刪除當前行
        {
            ...
            {
                const string sql_del = "UPDATE dbo.bs_payment Set state='2' WHERE id=@id";
                SqlConnection myCon = new SqlConnection(DBUtility.ConnectionString);
                myCon.Open();
                SqlTransaction myTrans = myCon.BeginTransaction();
                using (SqlCommand myCommand = new SqlCommand() { Connection = myCon, Transaction = myTrans })
        ...
        {
            dgvDetails.Focus();
            dtDetails = clsApp.GetDataTable("SELECT * FROM dbo.bs_payment Where state='0'");
