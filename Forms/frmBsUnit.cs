...
{
    public partial class frmBsUnit : DockContent

    {
        ...
        DataTable dtUnitKind = new DataTable();     
        ...
        private void BingDing_Data()
        {
            ...
            luekind.DataBindings.Add("EditValue", bds1, "kind");
        ...
        private void frmBsUnit_FormClosed(object sender, FormClosedEventArgs e)
        {
            dtUnitKind.Dispose();
        }
        ...
        {
            bool flag = false;
            string strSql = String.Format("Select '1' FROM bs_unit WHERE id='{0}'", txtid.Text.Trim());
        ...
        {
            dtDetail.Clear();
            const string sql = @"SELECT * From bs_unit with(nolock) Order by kind,id";           
        ...
        private void Save()  //保存新增或修改的資料
        {
            ...
            {
                if (dgvDetails.Rows[i].Cells["id"].Value.ToString() == "" || dgvDetails.Rows[i].Cells["name"].Value.ToString() == "" || dgvDetails.Rows[i].Cells["kind"].Value.ToString() == "")
                {
                    flag_datavalid = true;
            ...
            if (flag_datavalid)
            {
                MessageBox.Show("Unit id/name/kind cannot be empty !", myMsg.msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
